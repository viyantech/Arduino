...
void drawMenu() {
  ...
  // Split title into 2 lines to fit OLED
...
void playShooter() {
  ...
  if (millis() - enemyTimer > 120) {   // slower enemy
    enemyY++;
    enemyTimer = millis();
  }
...
void playCatch() {
  ...
  if (objY > 52 && abs(objX - playerX) < 10) { // Caught object
    score++;
    objY = 0;
    objX = random(0, 120);
    tone(BUZZER, 1200, 60);
  ...
  display.clearDisplay();
  display.fillRect(playerX, 56, 12, 6, WHITE); // player
  display.fillRect(objX, objY, 6, 6, WHITE);   // object
