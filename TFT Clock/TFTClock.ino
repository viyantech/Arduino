...
void drawClock() {
  ...
  // ---- Time String (4 spaces indent) ----
  char timeStr[25];
  if (blinkColon)
    sprintf(timeStr, "    %02d:%02d:%02d %s",
            hour12, minute, second, isPM ? "PM" : "AM");
  else
    sprintf(timeStr, "    %02d %02d %02d %s",
            hour12, minute, second, isPM ? "PM" : "AM");

  centerText(timeStr, 34, ST77XX_GREEN);

  // ---- Date String (1 space after weekday) ----
  char dateStr[30];
  sprintf(dateStr, "%s %02d/%02d/%04d",
          daysOfWeek[weekday], day, month, year);

  centerText(dateStr, 58, ST77XX_CYAN);
}

// -------- Centered Text Helper --------
void centerText(const char* text, int y, uint16_t color) {
  int16_t x1, y1;
  uint16_t w, h;
